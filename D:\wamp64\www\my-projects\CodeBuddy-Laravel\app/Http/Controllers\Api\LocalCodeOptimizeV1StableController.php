...
class LocalCodeOptimizeV1StableController extends Controller
{
    ...
    public function progress($code_register_id)
    {
        ...
        return view('smart.controllers-code-optimizer.progress', compact('code_register'));
    }
    ...
    public function module1($code_register_id)
    {
        ...
        foreach ($scopedMethods as $scopedMethodName => $scopedMethodCode) {
           ...
           if (in_array($scopedMethodName, $ignoreMethods)) {
            ...
            $helperLists = $this->getUsedHelperMethods($scopedMethodCode);

            $requestLists = $this->getUsedRequestMethods($scopedMethodCode);
            $scopedRequestCode = $this->step1request($scopedProject, $requestLists);

            // Step 2. Get Scoped Snippet Code (Inc Helpers) - VERIFIED
            if ($scopedProject->workflow == 'SPLCMT') {
                $snippetContent = $this->step2($scopedMethodName, $scopedMethodCode, $scopedHelperCode, $helperLists, $scopedRequestCode, $requestLists);
                // return dd($scopedMethods);
            } elseif ($scopedProject->workflow == 'TRAVERS') {
                $snippetContent = $this->step2Travers($scopedMethodName, $scopedMethodCode, $scopedHelperCode, $helperLists, $scopedRequestCode, $requestLists);
                // return dd($scopedMethods);
            } else {
                echo 'FLOW 404 </br>';
                return;
        ...
        if (
            ...
            $scopedMethodTotalCount = CodeRegisterFile::whereCodeRegisterId($scopedProject->id)->count();
    ...
    public function module5($code_register_id)
    {
        ...
        if (!$scopedProjectFile) {
            ...
            $scopedProjectFiles = CodeRegisterFile::whereCodeRegisterId($code_register_id)->get();
    ...
    public function clearProgress($code_register_id)
    {
        ...
        $scopedProject->status = 0;
        $scopedProject->files = 0;
        $scopedProject->save();
        ...
        CodeRegisterFileLog::whereCodeRegisterId($code_register_id)->delete();
    ...
    private function step2($scopedMethodName, $scopedMethodCode, $scopedHelperCode, $helperLists, $scopedRequestCode, $requestLists)
    {
        ...
        // Loop through each helper name
        foreach ($helperLists as $helperName) {
            $result .= "\n/**start-hqai-h-$helperName**/\n";
            $result .= $this->getScopedHelperMethodCode($scopedHelperCode, $helperName);
            $result .= "\n/**end-hqai-h-$helperName**/\n";
            $result .= "\n\n";
        }

        // Loop through each request name
        foreach ($requestLists as $requestName) {
            $result .= "\n/**start-hqai-r-$requestName**/\n";
            $result .= $scopedRequestCode;
            $result .= "\n/**end-hqai-r-$requestName**/\n";
            $result .= "\n\n";
        }
    ...
    private function step2Travers($scopedMethodName, $scopedMethodCode, $scopedHelperCode, $helperLists, $scopedRequestCode, $requestLists)
    {
        // Check if the 'method' parameter is blank
        if ($scopedMethodName == null || empty($scopedMethodCode)) {
            return response()->json(['message' => 'Method name is blank', 'status' => 400], 400);
