...
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Landing Pages', 'url' => 'javascript:void(0);', 'class' => 'active']];
        $authUser = auth()->user();
    @endphp
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>Landing Pages </h5>
                            <span>@lang('admin/ui.list_of') Landing Pages</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Landing Pages @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        <span class="font-weight-bold border-bottom trash-option   d-none ">Trash</span>
                        <div class="d-flex justicy-content-right">
                            @if (!is_null($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'],'add_landing_pages'))
                                <a href="{{ route('panel.member.landing-pages.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New Landing Page">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif
                            @if (!is_null($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'],'bulk_action_landing_pages'))
                    ...
                    <div id="ajax-container">
                        @include('panel.member.landing-pages.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.landing-pages.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'admin/landing-pages',
            'routeClass' => 'landing-pages',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.landing-pages.index') }}";
                fetchData(url);
