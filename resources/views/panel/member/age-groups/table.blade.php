<table id="table" class="table">
    <thead>
        <tr>
            @if (!isset($print_mode))
                @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_age_group') ||isUserHasPermission($authUser->permissions['permissions'], 'edit_age_group')||isUserHasPermission($authUser->permissions['permissions'], 'delete_age_group'))
                                    ...
                                    @if ($permissions->contains('edit_age_group') || $permissions->contains('delete_age_group'))
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            {{-- @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_age_group'))
                                                ...
                                                <a href="{{ route('panel.member.age-groups.show', secureToken($ageGroup->id)) }}"
                                                    title="Edit Age Group" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                                </a>
                                            @endif --}}
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_age_group'))
                                        
                                                <a href="{{ route('panel.member.age-groups.edit', secureToken($ageGroup->id)) }}"
                                                    title="Edit Age Group" class="dropdown-item ">
                                            ...
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_age_group'))
                                                ...
                                                <a href="{{ route('panel.member.age-groups.destroy', secureToken($ageGroup->id)) }}"
