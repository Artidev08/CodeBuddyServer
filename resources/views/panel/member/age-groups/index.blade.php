...
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Age Groups', 'url' => 'javascript:void(0);', 'class' => 'active']];
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>Age Groups </h5>
                            <span>@lang('admin/ui.list_of') Age Groups</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Age Groups @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        ...
                        <div class="d-flex justicy-content-right">
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_event'))
                                ...
                                <a href="{{ route('panel.member.age-groups.create') }}" class="btn btn-sm btn-outline-primary mr-2"
                                    title="Add New Age Group">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_age_group') ||isUserHasPermission($authUser->permissions['permissions'], 'delete_age_group'))
                                <div class="dropdown d-flex justicy-content-left">
                                    ...
                                    <ul class="dropdown-menu dropdown-position multi-level" role="menu"
                                        ...
                                        @endphp
                                        @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_age_group'))
                                        ...
                                        @endif
                                        <hr>
                                        @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_age_group'))
                                                <a href="javascript:void(0)" data-action="Move To Trash"
                                                    class="dropdown-item action trash-option text-danger fw-700"><i
                                                        class="ik ik-trash mr-2"></i> @lang('admin/ui.bulk_delete')</a>@endif
                    ...
                    <div id="ajax-container">
                        @include('panel.member.age-groups.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.age-groups.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/age-groups',
            'routeClass' => 'age-groups',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.age-groups.index') }}";
                fetchData(url);
