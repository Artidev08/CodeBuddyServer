                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex justicy-content-right">
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'add_sentiment'))
                            
                                <a href="{{ route('panel.member.sentiments.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New Sentiment">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif

                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_sentiment') || isUserHasPermission($authUser->permissions['permissions'], 'delete_sentiment'))    
                            ...
                            @endphp
                                @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_sentiment'))
                    ...
                    <div id="ajax-container">
                        @include('panel.member.sentiments.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.sentiments.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/sentiments',
            'routeClass' => 'sentiments',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.sentiments.index') }}";
                fetchData(url);
