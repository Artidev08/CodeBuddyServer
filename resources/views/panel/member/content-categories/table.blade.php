                ...
                <tr id="{{ $contentCategory->id }}">
                    @if (!isset($print_mode))
                                ...
                                @if ($permissions->contains('edit_content_category') || $permissions->contains('delete_content_category'))
                                    ...
                                    <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                        {{-- @if ($permissions->contains('view_content_categories'))
                                            <a href="{{ route('panel.member.content-categories.show', secureToken($contentCategory->id)) }}"
                                                title="Edit Content Category" class="dropdown-item ">
                                                <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                            </a>
                                        @endif --}}
                                        @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_content_category'))
                                        @if ($permissions->contains('edit_content_category'))
                                            <a href="{{ route('panel.member.content-categories.edit', secureToken($contentCategory->id)) }}"
                                                title="Edit Content Category" class="dropdown-item ">
                                        ...
                                        @if ($permissions->contains('delete_content_category'))
                                            <a href="{{ route('panel.member.content-categories.destroy', secureToken($contentCategory->id)) }}"
                        ...
                        <td class="text-center no-export"><a class="table-link"
                                href="{{ route('panel.member.content-categories.show', secureToken(@$contentCategory->id)) }}">
