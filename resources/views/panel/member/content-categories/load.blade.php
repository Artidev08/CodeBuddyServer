<div class="card-body">
        ...
        <div class="d-flex justify-content-between">
                ...
                <label for="">Show
                    <select name="length" class="length-input" id="length">
                        @foreach (tableLimits() as $limit)
                            <option
                                value="{{ $limit }}"{{ $contentCategories->perPage() == @$limit ? 'selected' : '' }}>
                                {{ $limit }}</option>
                        @endforeach
                    </select>
                    entries
            ...
            <div class="d-flex">
                ...
                {{--
                    ...
                    data-url="{{ route('admin.content-categories.print') }}"
                    data-rows="{{json_encode($contentCategories) }}"
    ...
    <div class="table-responsive">
        @include('panel.member.content-categories.table')
    </div>
</div>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="pagination mobile-justify-center">
                {{ $contentCategories->appends(request()->except('page'))->links() }}
            </div>
        </div>
        <div class="col-lg-4 mobile-mt-20">
            @if ($contentCategories->lastPage() > 1)
                <label class="d-flex justify-content-end mobile-justify-center" for="">
                    ...
                    <input type="number" class="w-25 form-control" id="jumpTo" name="page"
                        value="{{ $contentCategories->currentPage() ?? '' }}">
                </label>
            @endif
        </div>
    </div>
