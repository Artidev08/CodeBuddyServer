<table id="table" class="table">
    <thead>
        <tr>
            @if (!isset($print_mode))
                @if(!is_null($authUser->permissions)  && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'edit_event') || isUserHasPermission($authUser->permissions['permissions'], 'delete_event'))
                    <th class="no-export">
                ...
                <tr id="{{ $event->id }}">
                    @if (!isset($print_mode))
                                    ...
                                    @if ($permissions->contains('edit_event') || $permissions->contains('delete_event'))
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            {{-- @if ($permissions->contains('view_events'))
                                                <a href="{{ route('panel.member.events.show', secureToken($event->id)) }}"
                                                    title="Edit Event" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                                </a>
                                            @endif --}}
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_event'))
                                            
                                                @if (isset($occasion_id) && $occasion_id != null)
                                                <a href="{{ route('panel.member.events.edit', [secureToken($event->id),'occasion_id' => $occasion_id]) }}"
                                                    title="Edit Event" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-edit mr-2"></i>Edit</li>
                                                </a>
                                                @else
                                                <a href="{{ route('panel.member.events.edit', secureToken($event->id)) }}"
                                                    title="Edit Event" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-edit mr-2"></i>Edit</li>
                                                </a>
                                                @endif
                                            ...
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_event'))
                                                ...
                                                <a href="{{ route('panel.member.events.destroy', secureToken($event->id)) }}"
                                                    title="Delete Event"
                        ...
                        <td class="text-center no-export"><a class="table-link"
                                href="{{ route('panel.member.events.show', secureToken(@$event->id)) }}">
                                {{ @$event->getPrefix() }}
