...
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Media Types', 'url' => 'javascript:void(0);', 'class' => 'active']];
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>Media Types </h5>
                            <span>@lang('admin/ui.list_of') Media Types</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Media Types @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        <span class="font-weight-bold border-bottom trash-option   d-none ">Trash</span>
                        <div class="d-flex justicy-content-right">
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'add_media_type'))

                                <a href="{{ route('panel.member.media-types.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New Media Type">
                    ...
                    <div id="ajax-container">
                        @include('panel.member.media-types.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.media-types.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/media-types',
            'routeClass' => 'media-types',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.media-types.index') }}";
                fetchData(url);
