...
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Occasions', 'url' => 'javascript:void(0);', 'class' => 'active']];
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>Occasions </h5>
                            <span>@lang('admin/ui.list_of') Occasions</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Occasions @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        <span class="font-weight-bold border-bottom trash-option   d-none ">Trash</span>
                        <div class="d-flex justicy-content-right">
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'add_occasion'))
                                <a href="{{ route('panel.member.occasions.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New occasion">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'delete_occasion')|| isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_occasion'))
                                <div class="dropdown d-flex justicy-content-left">
                                    ...
                                    <ul class="dropdown-menu dropdown-position multi-level" role="menu"
                                        ...
                                        @endphp
                                        @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_occasion'))
                    ...
                    <div id="ajax-container">
                        @include('panel.member.occasions.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.occasions.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/occasions',
            'routeClass' => 'occasions',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.occasions.index') }}";
                fetchData(url);
