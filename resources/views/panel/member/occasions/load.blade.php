<div class="card-body">
        ...
        <div class="d-flex justify-content-between">
                ...
                <label for="">Show
                    <select name="length" class="length-input" id="length">
                        @foreach (tableLimits() as $limit)
                            <option
                                value="{{ $limit }}"{{ $occasions->perPage() == @$limit ? 'selected' : '' }}>
                                {{ $limit }}</option>
                        @endforeach
                    </select>
                    entries
            ...
            <div class="d-flex">
                ...
                {{--
                    ...
                    data-url="{{ route('admin.occasions.print') }}"
                    data-rows="{{json_encode($occasions) }}"
    ...
    <div class="table-responsive">
        @include('panel.member.occasions.table')
    </div>
</div>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="pagination mobile-justify-center">
                {{ $occasions->appends(request()->except('page'))->links() }}
            </div>
        </div>
        <div class="col-lg-4 mobile-mt-20">
            @if ($occasions->lastPage() > 1)
