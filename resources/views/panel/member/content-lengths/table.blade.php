<table id="table" class="table">
    <thead>
        <tr>
            @if (!isset($print_mode))
                @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'show_content_length') || isUserHasPermission($authUser->permissions['permissions'], 'edit_content_length')|| isUserHasPermission($authUser->permissions['permissions'], 'delete_content_length'))    
                    <th class="no-export">
                                ...
                                <div class="dropdown d-flex">
                                    ...
                                    @if ($permissions->contains('edit_content_length') || $permissions->contains('delete_content_length'))
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_content_length'))
                                            @if ($permissions->contains('view_content_lengths'))
                                                <a href="{{ route('panel.member.content-lengths.show', secureToken($contentLength->id)) }}"
                                                    title="Edit Content Length" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                                </a>
                                            @endif
                                            @endif
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_content_length'))
                                            @if ($permissions->contains('edit_content_length'))
                                                <a href="{{ route('panel.member.content-lengths.edit', secureToken($contentLength->id)) }}"
                                                    title="Edit Content Length" class="dropdown-item ">
                                            ...
                                            @if ($permissions->contains('delete_content_length'))
                                                <a href="{{ route('panel.member.content-lengths.destroy', secureToken($contentLength->id)) }}"
