...
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Content Lengths', 'url' => 'javascript:void(0);', 'class' => 'active']];
                ...
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            ...
                            <span>@lang('admin/ui.list_of') Content Lengths</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Content Lengths @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        ...
                        <div class="d-flex justicy-content-right">
                            ...
                            @if (
                                !is_null($authUser->permissions) &&
                                    ...
                                    isUserHasPermission($authUser->permissions['permissions'], 'add_content_length'))
                               
                                    <a href="{{ route('panel.member.content-lengths.create') }}"
                                        class="btn btn-sm btn-outline-primary mr-2" title="Add New Content Length">
                                        <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                    </a>
                            @endif
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_content_length') || isUserHasPermission($authUser->permissions['permissions'], 'delete_content_length'))    
                                    ...
                                    <ul class="dropdown-menu dropdown-position multi-level" role="menu"
                                        ...
                                        @if (
                                            !is_null($authUser->permissions) &&
                                                is_array($authUser->permissions) &&
                                                isset($authUser->permissions['permissions']) &&
                                                isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_content_length'))
                                        ...
                                        @if (
                                            !is_null($authUser->permissions) &&
                                                is_array($authUser->permissions) &&
                                                isset($authUser->permissions['permissions']) &&
                                                isUserHasPermission($authUser->permissions['permissions'], 'delete_content_length'))
                                            <hr>
                    ...
                    <div id="ajax-container">
                        @include('panel.member.content-lengths.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.content-lengths.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/content-lengths',
            'routeClass' => 'content-lengths',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.content-lengths.index') }}";
                fetchData(url);
