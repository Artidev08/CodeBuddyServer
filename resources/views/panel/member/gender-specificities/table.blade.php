<table id="table" class="table">
    <thead>
        <tr>
            @if (!isset($print_mode))
                @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'delete_gender_specificity') || isUserHasPermission($authUser->permissions['permissions'], 'edit_gender_specificity') ||isUserHasPermission($authUser->permissions['permissions'], 'show_gender_specificity'))
                                ...
                                <div class="dropdown d-flex">
                                    ...
                                    @if ($permissions->contains('edit_gender_specificity') || $permissions->contains('delete_gender_specificity'))
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_gender_specificity'))
                                        
                                                <a href="{{ route('panel.member.gender-specificities.show', secureToken($genderSpecificity->id)) }}"
                                                    title="Edit Gender Specificity" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                                </a>
                                            @endif
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_gender_specificity'))
                                        
                                                <a href="{{ route('panel.member.gender-specificities.edit', secureToken($genderSpecificity->id)) }}"
                                                    title="Edit Gender Specificity" class="dropdown-item ">
                                            ...
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_gender_specificity'))
                                                ...
                                                <a href="{{ route('panel.member.gender-specificities.destroy', secureToken($genderSpecificity->id)) }}"
