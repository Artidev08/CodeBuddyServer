...
@section('title', 'Gender Specificities')
@section('content')
    @php
        $breadcrumb_arr = [['name' => 'Gender Specificities', 'url' => 'javascript:void(0);', 'class' => 'active']];
                ...
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            ...
                            <span>@lang('admin/ui.list_of') Gender Specificities</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h3>Gender Specificities @if (request()->get('trash') == 1)
                                Trashed
                            @endif
                        </h3>
                        <span class="font-weight-bold border-bottom trash-option   d-none ">Trash</span>
                        <div class="d-flex justicy-content-right">
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'add_gender_specificity'))
                           
                                <a href="{{ route('panel.member.gender-specificities.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New Gender Specificity">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) && isUserHasPermission($authUser->permissions['permissions'], 'delete_gender_specificity') || isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_gender_specificity'))
                                <div class="dropdown d-flex justicy-content-left">
                                    ...
                                    @endphp
                                    @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'enable_disable_gender_specificity'))
                                        @foreach (getSelectValues($arrPrompt) as $key => $option)
                                        <li><a href="javascript:void(0)" class="dropdown-item action"
                                            data-action="is_ai_enabled-{{ $key }}">{{ $option }}</a></li>
                                        @endforeach
                                    @endif
                                    @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_gender_specificity'))   
                                    <hr>
                                        <a href="javascript:void(0)" data-action="Move To Trash"
                                            class="dropdown-item action trash-option text-danger fw-700"><i
                    ...
                    <div id="ajax-container">
                        @include('panel.member.gender-specificities.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.gender-specificities.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/gender-specificities',
            'routeClass' => 'gender-specificities',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.gender-specificities.index') }}";
                fetchData(url);
        ...
        <script>
            ...
            function html_table_to_excel(type) {
                ...
                var report_format = [{
                    ...
                    {
                        'label': "Report Name",
                        'value': "Gender Specificities Report"
                    },
                    {
                        'label': "Company",
                        'value': "{{ env('APP_NAME') }}"
