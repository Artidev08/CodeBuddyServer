                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex justicy-content-right">
                            ...
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'add_content'))
                                <a href="{{ route('panel.member.contents.create') }}"
                                    class="btn btn-sm btn-outline-primary mr-2" title="Add New Content">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang('admin/ui.add')
                                </a>
                            @endif
                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_content'))
                    ...
                    <div id="ajax-container">
                        @include('panel.member.contents.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.member.contents.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.member.include.more-action', [
            'actionUrl' => 'member/contents',
            'routeClass' => 'contents',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.member.contents.index') }}";
                fetchData(url);
