...
<script>
    window.onload = function() {
        ...
        fetchData("{{ route('panel.member.templates.index',['type' => request()->get('type')]) }}");
    };
</script>
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between mobile-d-block" style="margin-top: -10px">
                        ...
                        <ul class="nav nav-pills custom-pills" id="pills-tab" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link @if (!request()->has('type') || request()->get('type') == 1) active @endif"
                                    href="{{ route('panel.member.templates.index', ['type' => 1]) }}"> @lang('admin/ui.blog_mail_template') </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link @if (request()->has('type') && request()->get('type') == 2) active @endif"
                                    href="{{ route('panel.member.templates.index', ['type' => 2]) }}"> @lang('admin/ui.blog_sms_template') </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link @if (request()->has('type') && request()->get('type') == 3) active @endif"
                                    href="{{ route('panel.member.templates.index', ['type' => 3]) }}"
                                    role="tab"
                            ...
                            <li class="nav-item">
                                <a class="nav-link @if (request()->has('type') && request()->get('type') == 4) active @endif"
                                    href="{{ route('panel.member.templates.index', ['type' => 4]) }}"
                                    role="tab"
                                    aria-controls="pills-timeline"
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <form action="{{ route('panel.member.templates.index') }}" class="d-flex ajaxForm"
                                method="GET" id="TableForm">

                            <div class="dropdown mt-6 mb-0">

                                @if ($permissions->contains('add_mail_template'))
                                    <a href="{{ route('panel.member.templates.create') }}"
                        ...
                        <form action="{{ route('panel.member.templates.bulk-action') }}" method="POST"
                                id="bulkAction">
                            @csrf
                ...
                <div id="ajax-container" style="display: none;">
                    @include('panel.member.mail-sms-templates.load')
                </div>
            </div>
        </div>
        @include('panel.member.modal.sitemodal', [
            'title' => 'How to use',
            'content' =>
...
@if (getSetting('templates_table_filter', @$setting))
    @include('panel.member.mail-sms-templates.include.filter')
@endif
...
@push('script')
    ...
    <script>
        $('#reset').click(function () {
            fetchData("{{ route('panel.member.templates.index') }}");
            window.history.pushState("", "", "{{ route('panel.member.templates.index') }}");
