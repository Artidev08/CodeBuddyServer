<table id="table" class="table">
    <thead>
        <tr>
            @if (!isset($print_mode))
                @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_badge') || isUserHasPermission($authUser->permissions['permissions'], 'edit_badge') || isUserHasPermission($authUser->permissions['permissions'], 'delete_badge'))
                                ...
                                <div class="dropdown d-flex">
                                    ...
                                    @if ($permissions->contains('edit_badge') || $permissions->contains('delete_badge'))
                                        ...
                                        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'show_badge'))
                                            
                                                <a href="{{ route('panel.member.badges.show', secureToken($badge->id)) }}"
                                                    title="Edit Badge" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                                </a>
                                            @endif
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'edit_badge'))
                                        
                                                <a href="{{ route('panel.member.badges.edit', secureToken($badge->id)) }}"
                                                    title="Edit Badge" class="dropdown-item ">
                                                    <li class="p-0"><i class="ik ik-edit mr-2"></i>Edit</li>
                                                </a>
                                            @endif
                                            @if(!is_null($authUser->permissions) && is_array($authUser->permissions) && isset($authUser->permissions['permissions']) &&  isUserHasPermission($authUser->permissions['permissions'], 'delete_badge'))
                                            
                                                <a href="{{ route('panel.member.badges.destroy', secureToken($badge->id)) }}"
                                                    title="Delete Badge"
