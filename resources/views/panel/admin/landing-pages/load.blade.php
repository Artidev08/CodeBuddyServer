<div class="card-body">
        ...
        <div class="d-flex justify-content-between">
                ...
                <label for="">Show
                    <select name="length" class="length-input" id="length">
                        @foreach (tableLimits() as $limit)
                            <option
                                value="{{ $limit }}"{{ $landingPages->perPage() == @$limit ? 'selected' : '' }}>
                                {{ $limit }}</option>
                        @endforeach
                    </select>
                    entries
            ...
            <div class="d-flex">
                ...
                {{--
                    ...
                    data-url="{{ route('admin.landing-pages.print') }}"
                    data-rows="{{json_encode($landingPages) }}"
                ...
                <div id="recent_searches" class="d-flex">
                    {{-- @if (!empty(request()->all()))
                        @foreach (request()->all() as $key => $value)
                            @if ($key != '_')
                                <a href="javascript:void(0)" id="recent_{{ $key }}"
                                    onclick="getTableContent('{{ $key }}')"
                                    class="badge mt-1 ml-1 custom-badge">{{ ucwords(str_replace('_', ' ', $key)) }} <i
    ...
    <div class="table-responsive">
        @include('panel.admin.landing-pages.table')
    </div>
</div>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="pagination mobile-justify-center">
                {{ $landingPages->appends(request()->except('page'))->links() }}
            </div>
        </div>
        <div class="col-lg-4 mobile-mt-20">
            @if ($landingPages->lastPage() > 1)
                <label class="d-flex justify-content-end mobile-justify-center" for="">
                    ...
                    <input type="number" class="w-25 form-control" id="jumpTo" name="page"
                        value="{{ $landingPages->currentPage() ?? '' }}">
                </label>
            @endif
        </div>
    </div>
