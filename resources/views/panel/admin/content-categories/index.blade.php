                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex justicy-content-right">
                            @if ($permissions->contains('add_content_category'))
                                <a href="{{ route('panel.admin.content-categories.create') }}"
                    ...
                    <div id="ajax-container">
                        @include('panel.admin.content-categories.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.admin.content-categories.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.admin.include.more-action', [
            'actionUrl' => 'admin/content-categories',
            'routeClass' => 'content-categories',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.admin.content-categories.index') }}";
                fetchData(url);
