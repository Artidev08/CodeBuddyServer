                        ...
                        <td class="no-export">
                            <div class="dropdown d-flex">
                                <input type="checkbox" class="mr-2 text-center" name="id" onclick="countSelected()"
                                    value="{{ $relation->id }}">
                                @if ($permissions->contains('edit_relation') || $permissions->contains('delete_relation'))
                                    ...
                                    <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                        @if ($permissions->contains('view_relations'))
                                            <a href="{{ route('panel.admin.relations.show', secureToken($relation->id)) }}"
                                                title="Edit Relation" class="dropdown-item ">
                                                <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                            </a>
                                        @endif
                                        @if ($permissions->contains('edit_relation'))
                                            <a href="{{ route('panel.admin.relations.edit', secureToken($relation->id)) }}"
                                                title="Edit Relation" class="dropdown-item ">
                                                <li class="p-0"><i class="ik ik-edit mr-2"></i>Edit</li>
                                            </a>
                                        @endif
                                        @if ($permissions->contains('delete_relation'))
                                            <a href="{{ route('panel.admin.relations.destroy', secureToken($relation->id)) }}"
