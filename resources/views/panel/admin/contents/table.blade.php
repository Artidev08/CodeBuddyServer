                        ...
                        <td class="no-export">
                            <div class="dropdown d-flex">
                                <input type="checkbox" class="mr-2 text-center" name="id" onclick="countSelected()"
                                    value="{{ $content->id }}">
                                @if ($permissions->contains('edit_content') || $permissions->contains('delete_content'))
                                    ...
                                    <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                        {{-- @if ($permissions->contains('view_contents'))
                                            <a href="{{ route('panel.admin.contents.show', secureToken($content->id)) }}"
                                                title="Edit Content" class="dropdown-item ">
                                                <li class="p-0"><i class="ik ik-eye mr-2"></i>Show</li>
                                            </a>
                                        @endif --}}
                                        @if ($permissions->contains('edit_content'))
                                            <a href="{{ route('panel.admin.contents.edit', secureToken($content->id)) }}"
                                                title="Edit Content" class="dropdown-item ">
                                                <li class="p-0"><i class="ik ik-edit mr-2"></i>Edit</li>
                                            </a>
                                        @endif
                                        @if ($permissions->contains('delete_content'))
                                            <a href="{{ route('panel.admin.contents.destroy', secureToken($content->id)) }}"
