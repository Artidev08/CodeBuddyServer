                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex justicy-content-right">
                            {{-- @if ($permissions->contains('add_occasion')) --}}
                            <a href="{{ route('panel.admin.occasions.create') }}"
                    ...
                    <div id="ajax-container">
                        @include('panel.admin.occasions.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.admin.occasions.includes.filter')
    <!-- push external js -->
    @push('script')
        @include('panel.admin.include.more-action', [
            'actionUrl' => 'admin/occasions',
            'routeClass' => 'occasions',
        ])
        <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
        <script>
            $('#reset').click(function() {
                let url = "{{ route('panel.admin.occasions.index') }}";
                fetchData(url);
