    ...
    @php
        $breadcrumb_arr = [
            [
                ...
                'url' => route('panel.admin.users.index', ['role' => 'Member']),
                'class' => '',
            ],
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="d-flex align-items-center">
                            <a href="{{ route('panel.admin.agent-content-registers.destroy', [0, 'agent_id' => request()->agent_id, 'type' => 'all']) }}"
                                class="dropdown-item  delete-item btn btn-sm btn-outline-danger mr-2" title="Reset All Registers" data-message="Do you want to delete this agent's registers?"> @lang('admin/ui.reset_registers')
                            </a>
                            <a href="{{ route('panel.admin.agent-content-registers.create', ['agent_id' => request()->agent_id]) }}"
                            ...
                            <form action="{{ route('panel.admin.agent-content-registers.bulk-action') }}" method="POST"
                                id="bulkAction" class="">
                                @csrf
                                ...
                                <div>
                                    ...
                                    <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
                                        {{-- @if (getSetting('user_management_bulk_delete', @$setting)) --}}
                                        {{-- <hr class="m-1"> --}}
                    ...
                    <div id="ajax-container">
                        @include('panel.admin.agent-content-registers.load')
                    </div>
                </div>
            </div>
        </div>
    </div>
    @include('panel.admin.agent-content-registers.include.filter')
    ...
    @push('script')
        ...
        <script>
            $('#reset').click(function() {
                fetchData("{{ route('panel.admin.agent-content-registers.index') }}");
                window.history.pushState("", "", "{{ route('panel.admin.agent-content-registers.index') }}");
        ...
        <script>
            function changeStatus(status, id, element) {
                ...
                $.ajax({
                    url: '{{ route('panel.admin.agent-content-registers.update-status') }}',
                    method: 'POST',
                    data: {
                        status: status,
                        id: id,
