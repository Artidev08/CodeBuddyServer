...
@section('title', $websiteEnquiry->getPrefix() . ' ' . $label . ' Edit')
@section('content')

    @php
        @$breadcrumb_arr = [
            ['name' => $label, 'url' => route('panel.admin.website-enquiries.index'), 'class' => ''],
            [
                'name' => $websiteEnquiry->getPrefix(),
                'url' => route('panel.admin.website-enquiries.index'),
                'class' => '',
            ],
    ...
    <div class="container-fluid">
        ...
        <form action="{{ route('panel.admin.website-enquiries.update', @$websiteEnquiry->id ?? '') }}" method="post"
            class="ajaxForm">
            @csrf
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group {{ @$errors->has('subject') ? 'has-error' : '' }}">
                                                <x-label name="subject" validation="enquiry_subject"
                                                    tooltip="edit_enquiry_subject" />
                                                <x-input name="subject" placeholder="Enter Subject" type="text"
                                                    tooltip="edit_enquiry_subject" regex="subject"
                                                    validation="enquiry_subject"
                                                    value="{{ @$websiteEnquiry->subject ?? '' }}" />
                                        ...
                                        <div class="col-sm-12">
                                            <div class="form-group {{ @$errors->has('description') ? 'has-error' : '' }}">
                                                <x-label name="description" validation="enquiry_description"
                                                    tooltip="edit_enquiry_description" />
                                                <x-textarea regex="short_description" validation="enquiry_description"
                                                    value="{{ @$websiteEnquiry->description ?? '--' }}" name="description"
                ...
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <h3> @lang('admin/ui.customer_info')</h3>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-12 col-md-12">
                                    <div class="form-group {{ @$errors->has('name') ? 'has-error' : '' }}">
                                        <x-label name="full_name" validation="enquiry_name" tooltip="edit_enquiry_name" />
                                        <x-input name="name" placeholder="Enter Name" type="text"
                                            tooltip="edit_enquiry_name" regex="name" validation="common_name"
                                            value="{{ @$websiteEnquiry->name }}" />

                                    </div>
                                </div>
                                <div class="col-sm-12 col-md-12">
                                    <div class="form-group {{ @$errors->has('phone') ? 'has-error' : '' }}">
                                        <x-label name="phone" validation="" tooltip="edit_enquiry_phone" />
                                        {{-- <x-input name="phone" placeholder="Enter Phone" type="number"
                                        tooltip="add_enquiry_phone" regex="" validation=""
                                        value="{{ @$websiteEnquiry->phone }}" /> --}}
                                        <div class="input-group">
                                            <input type="hidden" id="countryCodeInput" name="country_code" value="">
                                            <input type="tel" class="form-control"
                                                id="phone" name="phone" value="{{ $websiteEnquiry->fullPhone() }}">
                                        </div>
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <div class="form-group {{ @$errors->has('email') ? 'has-error' : '' }}">
                                        ...
                                        <x-input name="email" placeholder="{{ __('admin/ui.enter') . ' ' . __('admin/ui.email') }}" type="email"
                                            tooltip="add_enquiry_email" regex="email" validation=""
                                            value="{{ @$websiteEnquiry->email ?? '' }}" />
