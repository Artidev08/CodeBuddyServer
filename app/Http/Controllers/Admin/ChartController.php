...
class ChartController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if ($request->ajax()) {
            return view('admin.charts.load', ['charts' => $charts,'projects'=>$projects])->render();
        }
        return view('admin.charts.index', compact('charts','projects','divideLogics','divideLogicType'));
    }

    public function logs(Chart $chart,$record_type,Request $request)
    {
        if(empty($chart))
        ...
        if ($request->has('status') && $request->get('status') != null) {
            $logs->whereStatus([request()->get('status')]);
        }
        if ($request->get('search')) {
            $logs->where(
        ...
        $logs = $logs->where('related_type',Chart::class)->where('related_id',$chart->id)
        ->where('record_type',$record_type)
        ->latest()->latest()->paginate($length);

        if ($request->ajax()) {
            return view('admin.charts.logs.load', ['logs' => $logs,'chart'=>$chart,'record_type' =>$record_type])->render();
        }
        return view('admin.charts.logs.index', compact('logs','chart','record_type'));
    }
