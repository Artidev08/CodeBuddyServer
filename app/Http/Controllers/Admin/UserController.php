...
class UserController extends Controller
{
    ...
    public function store(UserRequest $request)
    {
        ...
        try {
            ...
            $user = User::create(
                ...
                'parked_limit'    => $request->parked_limit??User::PARKED_LIMIT_5,
                'chart_gap'    => $request->chart_gap??0,
                'chart_limit'    => $request->chart_limit??0,
                'dx_limit'    => $request->dx_limit??0,
                'phone'    => $request->phone,
                'wallet'    => 0, // Opening with zero balance
                'payload'    => $request->payload, // Opening with zero balance
            ...
            //     $folders = Folder::where('created_by', 1)->get();
            //     foreach ($folders as $folder) {
            //         $replica = $folder->replicate();
            //         $replica->created_by = $user->id;
            //         $replica->save();
            //     }
            // }
            if (request()->ajax()) {
        ...
        try {
            if ($id == auth()->id()) {
                ...
                if($user->status == 1){
                    ...
                    // Redirect.
                    if ($authRole == 'Entry') {
                        return redirect(route('entry.dashboard.index'));
                    }
                    if ($authRole == 'Auditor') {
                    ...
                    return back()->with('error', 'Your account is inactive please contact to admin.');
                }
    ...
    public function update(Request $request, User $user)
    {
        ...
        try {
            ...
            $user->parked_limit = $request->parked_limit??0;
            $user->chart_gap = $request->chart_gap??0;
            $user->chart_limit = $request->chart_limit??0;
            $user->dx_limit = $request->dx_limit??0;
        ...
        try {
            ...
            switch ($request->action) {
                ...
                case ('permission'):
                    ...
                    return back()->with('User update successfully');
